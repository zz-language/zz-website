...
export function renderAuth(): void {
  ...
  slot.innerHTML =
    ...
    `<div id="user-menu" class="user-menu hidden absolute right-0 top-[calc(100%+8px)] w-60 rounded-xl border border-border bg-surface shadow-2xl shadow-black/50 p-1.5 text-sm z-50">` +
    `<div class="px-3 py-2.5 border-b border-border mb-1"><div class="text-[11px] text-faint">Signed in as</div>` +
    `<div class="font-mono font-medium text-foreground truncate">${escName(session.login)}</div></div>` +
